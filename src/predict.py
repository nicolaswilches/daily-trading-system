...
def run_prediction(ticker):
    ...
    # SimFin ticker mapping (UI → API)
    search_ticker = ticker
    if ticker == "GOOGL":
        search_ticker = "GOOG"
    elif ticker == "GOOG":
        search_ticker = "GOOGL"  # SimFin uses GOOGL, not GOOG
    elif ticker == "META":
        search_ticker = "META"  # Try META first, fallback handled in except block
    ...
    try:
        ...
        if "429" in str(e):
            return {
                "error": "API Limit Reached. Please try again tomorrow (Free tier quota exhausted)."
            }
        return {"error": f"API Error: {e}"}

    if df_prices is None or len(df_prices) < 200:
        return {
            "error": f"Insufficient data for {ticker} (fetched as {search_ticker})."
        }
    ...
    df = df.with_columns(
        [
            ...
            (pl.col("Close") / pl.col("Close").rolling_mean(20) - 1).alias(
                "Dist_SMA_20"
            ),
            (pl.col("Close") / pl.col("Close").rolling_mean(50) - 1).alias(
                "Dist_SMA_50"
            ),
            (pl.col("Close") / pl.col("Close").rolling_mean(200) - 1).alias(
                "Dist_SMA_200"
            ),
            (
                (pl.col("Close").diff() > 0).cast(pl.Int32).rolling_mean(14)
                / (pl.col("Close").diff().abs().rolling_mean(14) + 1e-9)
            ).alias("RSI_Proxy"),
